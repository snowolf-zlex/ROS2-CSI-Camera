...
class StereoCameraRTSPServer:
    ...
    def __init__(self):
        """
        初始化函数，设置GStreamer和RTSP服务器，配置两个摄像头并启动服务。
        """
        Gst.init(None)
        ...
        # 设置左侧摄像头
        self._setup_camera(sensor_id=0, mount_point='/left')
        # 设置右侧摄像头
        self._setup_camera(sensor_id=1, mount_point='/right')

        # 将服务器连接到主循环并启动
        self.server.attach(None)
        print("RTSP流已准备就绪:")
        print(f"rtsp://localhost:8554/left")
        print(f"rtsp://localhost:8554/right")
